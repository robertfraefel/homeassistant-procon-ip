...
def _generate_dashboard_yaml(coordinator: ProConIPCoordinator) -> str:
    ...
    return (
        ...
        f"        cards:\n"
        f"\n"
        f"          - type: custom:apexcharts-card\n"
        f"            chart_type: radialBar\n"
        f"            header:\n"
        f"              show: true\n"
        f"              title: Water Temperature\n"
        f"              show_states: true\n"
        f"              colorize_states: true\n"
        f"            series:\n"
        f"              - entity: sensor.{d}_pool\n"
        f"                min: 0\n"
        f"                max: 40\n"
        f"                color: \"{{% set v = states('sensor.{d}_pool') | float(20) %}}{{% if v < 28 %}}#4caf50{{% elif v < 34 %}}#ffc107{{% else %}}#f44336{{% endif %}}\"\n"
        f"\n"
        f"          - type: custom:apexcharts-card\n"
        f"            chart_type: radialBar\n"
        f"            header:\n"
        ...
        f"                min: 5.0\n"
        f"                max: 9.0\n"
        f"                color: \"{{% set v = states('sensor.{d}_ph') | float(7.2) %}}{{% if v < 6.5 %}}#f44336{{% elif v < 7.0 %}}#ffc107{{% elif v < 7.4 %}}#4caf50{{% elif v < 8.0 %}}#ffc107{{% else %}}#f44336{{% endif %}}\"\n"
        f"\n"
        f"          - type: custom:apexcharts-card\n"
        f"            chart_type: radialBar\n"
        f"            header:\n"
        ...
        f"                min: 0\n"
        f"                max: 1000\n"
        f"                color: \"{{% set v = states('sensor.{d}_redox') | float(700) %}}{{% if v < 550 %}}#f44336{{% elif v < 650 %}}#ffc107{{% elif v < 750 %}}#4caf50{{% elif v < 850 %}}#ffc107{{% else %}}#f44336{{% endif %}}\"\n"
        f"\n"
